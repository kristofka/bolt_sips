  ...
  defmacro __using__(_options) do
    quote do
      ...
      defp do_call_encode(:string, string, bolt_version)
           when bolt_version <= 3 and byte_size(string) <= 15 do
        [<<@tiny_bitstring_marker::4, byte_size(string)::4>>, string]
      end

      defp do_call_encode(:string, string, bolt_version)
           when bolt_version <= 3 and byte_size(string) <= 255 do
        [<<@bitstring8_marker, byte_size(string)::8>>, string]
      end

      defp do_call_encode(:string, string, bolt_version)
           when bolt_version <= 3 and byte_size(string) <= 65_535 do
        [<<@bitstring16_marker, byte_size(string)::16>>, string]
      end

      defp do_call_encode(:string, string, bolt_version)
           when bolt_version <= 3 and byte_size(string) <= 4_294_967_295 do
        [<<@bitstring32_marker, byte_size(string)::32>>, string]
      end

      # Integer
      defp do_call_encode(:integer, integer, bolt_version)
           when bolt_version <= 3 and integer in -16..127 do
        <<integer>>
      end

      defp do_call_encode(:integer, integer, bolt_version)
           when bolt_version <= 3 and integer in @int8 do
        <<@int8_marker, integer>>
      end
